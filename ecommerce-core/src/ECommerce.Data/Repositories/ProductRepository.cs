...
public class ProductRepository : Repository<Product>, IProductRepository
{
    ...
    public async Task<(IEnumerable<Product> Products, int TotalCount)> GetProductsByCategoryAsync(int categoryId, int pageNumber = 1, int pageSize = 10, string sortBy = "Name", string sortDirection = "ASC")
    {
        ...
        var products = await _context.Products.FromSqlRaw("EXEC GetProductsByCategory @CategoryId, @PageNumber, @PageSize, @SortBy, @SortDirection", categoryParam, pageNumberParam, pageSizeParam, sortByParam, sortDirectionParam).Include(p => p.Category).ToListAsync();
        ...
        var totalCount = await _context.Database.SqlQueryRaw<int>("SELECT COUNT(*) FROM Products WHERE CategoryId = @CategoryId AND IsActive = 1", totalCountParam).FirstOrDefaultAsync();
        return (products, totalCount);
    }

    public async Task<IEnumerable<Product>> GetProductsByPriceRangeAsync(decimal minPrice, decimal maxPrice)
    {
        return await _dbSet.Where(p => p.Price >= minPrice && p.Price <= maxPrice && p.IsActive).Include(p => p.Category).OrderBy(p => p.Price).ToListAsync();
    }
    ...
    public async Task<IEnumerable<Product>> SearchProductsAsync(string searchTerm)
    {
        ...
        return await _context.Products.FromSqlRaw(@"
                ...
                AND p.IsActive = 1
                ORDER BY p.Name", searchParam).Include(p => p.Category).ToListAsync();
    }

    public async Task<IEnumerable<Product>> GetTopSellingProductsAsync(int count = 10, int daysBack = 30)
    {
        var cutoffDate = DateTime.UtcNow.AddDays(-daysBack);
        return await _context.Products.Where(p => p.IsActive).Select(p => new { Product = p, SalesCount = p.OrderItems.Where(oi => oi.Order.OrderDate >= cutoffDate && oi.Order.Status == "Delivered").Sum(oi => oi.Quantity) }).OrderByDescending(x => x.SalesCount).Take(count).Select(x => x.Product).Include(p => p.Category).ToListAsync();
    }
    ...
    public async Task<IEnumerable<Product>> GetProductsWithLowStockAsync(int threshold = 10)
    {
        ...
        return await _dbSet.Where(p => p.IsActive).Include(p => p.Category).Take(5) // Placeholder - would be actual stock check
        .ToListAsync();
    }
    ...
    {
        return await _dbSet.CountAsync(p => p.CategoryId == categoryId && p.IsActive);
    }
    ...
    public async Task<IEnumerable<dynamic>> GetProductSalesAnalysisAsync(int? categoryId = null, int daysBack = 90)
    {
        ...
        var results = await _context.Database.SqlQueryRaw<dynamic>(@"
                SELECT * FROM dbo.GetProductSalesAnalysis(@CategoryId, @DaysBack)
                ORDER BY TotalRevenue DESC", categoryParam, daysBackParam).ToListAsync();
        return results;
    }
    ...
    public async Task<int> BulkUpdatePricesAsync(int categoryId, decimal priceMultiplier)
    {
        ...
        return await _context.Database.ExecuteSqlRawAsync(@"
                ...
                WHERE CategoryId = @CategoryId AND IsActive = 1", categoryParam, multiplierParam);
    }
    ...
    {
        return await _dbSet.Include(p => p.Category).FirstOrDefaultAsync(p => p.ProductId == id);
    }

    public override async Task<IEnumerable<Product>> GetAllAsync()
    {
        return await _dbSet.Include(p => p.Category).Where(p => p.IsActive).OrderBy(p => p.Name).ToListAsync();
    }
}
