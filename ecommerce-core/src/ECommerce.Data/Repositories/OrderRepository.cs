...
public class OrderRepository : Repository<Order>, IOrderRepository
{
    ...
    {
        IQueryable<Order> query = _context.Orders.Include(o => o.Customer).Include(o => o.OrderItems).ThenInclude(oi => oi.Product).OrderByDescending(o => o.OrderDate);
    ...
    public async Task<int> ProcessOrderAsync(int customerId, string orderItemsJson, string? shippingAddress = null)
    {
        ...
        await _context.Database.ExecuteSqlRawAsync("EXEC ProcessOrder @CustomerId, @OrderItemsJson, @ShippingAddress, @OrderId OUTPUT", customerParam, orderItemsParam, shippingParam, orderIdParam);
        return (int)orderIdParam.Value;
    }

    public async Task<Order?> GetOrderWithDetailsAsync(int orderId)
    {
        return await _dbSet.Include(o => o.Customer).Include(o => o.OrderItems).ThenInclude(oi => oi.Product).ThenInclude(p => p.Category).FirstOrDefaultAsync(o => o.OrderId == orderId);
    }

    public async Task<IEnumerable<Order>> GetCustomerOrdersAsync(int customerId, int? limit = null)
    {
        IQueryable<Order> query = _dbSet.Where(o => o.CustomerId == customerId).Include(o => o.OrderItems).ThenInclude(oi => oi.Product).OrderByDescending(o => o.OrderDate);
        if (limit.HasValue)
            query = query.Take(limit.Value);
        return await query.ToListAsync();
    }
    ...
    public async Task<bool> UpdateOrderStatusAsync(int orderId, string newStatus)
    {
        ...
        var rowsAffected = await _context.Database.ExecuteSqlRawAsync(@"
            ...
            WHERE OrderId = @OrderId", orderParam, statusParam);
    ...
    public async Task<bool> CancelOrderAsync(int orderId, string reason)
    {
        ...
        var rowsAffected = await _context.Database.ExecuteSqlRawAsync(@"
            ...
            VALUES ('Orders', 'CANCELLED', @OrderId, SUSER_SNAME(), GETUTCDATE(), @Reason);", orderParam, reasonParam);
        return rowsAffected > 0;
    }
    ...
    public async Task<decimal> CalculateOrderTotalAsync(int orderId)
    {
        ...
        var orderParam = new SqlParameter("@OrderId", orderId);
        var subTotal = await _context.Database.SqlQueryRaw<decimal>(@"SELECT ISNULL(SUM(LineTotal), 0) FROM OrderItems WHERE OrderId = @OrderId", orderParam).FirstOrDefaultAsync();
        // Use the CalculateOrderTotal function
        var totalParam = new SqlParameter("@SubTotal", subTotal);
        var total = await _context.Database.SqlQueryRaw<decimal>("SELECT dbo.CalculateOrderTotal(@SubTotal, DEFAULT, DEFAULT)", totalParam).FirstOrDefaultAsync();
        return total;
    }

    public async Task<IEnumerable<Order>> GetOrdersByDateRangeAsync(DateTime startDate, DateTime endDate)
    {
        return await _dbSet.Where(o => o.OrderDate >= startDate && o.OrderDate <= endDate).Include(o => o.Customer).OrderByDescending(o => o.OrderDate).ToListAsync();
    }

    public async Task<IEnumerable<Order>> GetOrdersByStatusAsync(string status)
    {
        return await _dbSet.Where(o => o.Status == status).Include(o => o.Customer).OrderByDescending(o => o.OrderDate).ToListAsync();
    }

    public async Task<IEnumerable<Order>> GetHighValueOrdersAsync(decimal minimumAmount, int daysBack = 30)
    {
        var cutoffDate = DateTime.UtcNow.AddDays(-daysBack);
        return await _dbSet.Where(o => o.TotalAmount >= minimumAmount && o.OrderDate >= cutoffDate).Include(o => o.Customer).OrderByDescending(o => o.TotalAmount).ToListAsync();
    }
    ...
    public async Task<dynamic> GetCustomerOrderHistoryAsync(int customerId, DateTime? startDate = null, DateTime? endDate = null, bool includeDetails = false)
    {
        ...
        var result = await _context.Database.SqlQueryRaw<dynamic>(@"
            EXEC GetCustomerOrderHistory @CustomerId, @StartDate, @EndDate, @IncludeDetails", customerParam, startParam, endParam, detailsParam).ToListAsync();
        return result;
    }
    ...
    public async Task<IEnumerable<dynamic>> GetSalesTrendsAsync(int daysBack = 30)
    {
        ...
        var trends = await _context.Database.SqlQueryRaw<dynamic>(@"
            ...
            ORDER BY CAST(OrderDate AS DATE)", new SqlParameter("@CutoffDate", cutoffDate)).ToListAsync();
        return trends;
    }
    ...
    public async Task<IEnumerable<dynamic>> GetTopCustomersByOrderValueAsync(int count = 10)
    {
        var topCustomers = await _context.Database.SqlQueryRaw<dynamic>(@"
            ...
            ORDER BY SUM(o.TotalAmount) DESC", new SqlParameter("@Count", count)).ToListAsync();
        return topCustomers;
    }
    ...
    {
        return await _dbSet.Include(o => o.Customer).Include(o => o.OrderItems).ThenInclude(oi => oi.Product).FirstOrDefaultAsync(o => o.OrderId == id);
    }
}
